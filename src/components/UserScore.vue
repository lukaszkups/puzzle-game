...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateHighScores() {
        return axios.get('http://localhost:3000/scores?_sort=score&_order=asc&_limit=10').then((data) => {
          this.$store.dispatch('updateHighScore', data.data);
          this.$store.dispatch('updateUserScoreVisibility', false);
        });
      },
    },
  };
